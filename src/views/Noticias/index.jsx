...
const Noticias = () => {
  return (
    <div>
      <Header title="Comunidad / Noticias"/>
...
const Caso = ({ name, content ="", img="/src/assets/imagenIcon.png" }) => {
    ...
    <>
      <div className="flex flex-col border-2 border-orange-400 rounded-lg">
        <div className="w-full h-72 border flex shadow-lg md:border-2 mx-0 border-slate-500 rounded-lg overflow-hidden">
          <img className='h-full w-max self-center py-auto mx-auto' src={img} alt="" />
        </div>
        <div className=" my-4 px-3">
          <p className="text-2xl font-bold mb-4">{name}</p>
          {contentArray.map((fragmento, index) => (
            <p key={index} className="text-xl font-extralight text-slate-700">{fragmento}</p>
          ))}
        </div>
